...
调整宽度后用 `prefix+shift+m` 刷新。`gauges` 下 `cx` 行也有自己的严重程度配色，
...
| Layout | `gauges`（默认）在每个额度数字旁加进度条；`packed` 合并相关字段；`stacked` 将字段分行显示；`compact` 先显示模型，再把权限模式图标、上下文进度条和输入/输出 token 放在一行，最后是缓存和 TTL（进度条需要约 32 列宽的侧栏） |
...
| 调整宽度后 `gauges` 仍是旧长度 | 用 `prefix+shift+m` 刷新；没有随拖动实时发布的路径 |
