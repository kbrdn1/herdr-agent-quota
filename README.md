...
| Layout | `gauges` (default) adds a meter beside each quota number; `packed` groups related fields; `stacked` gives each field a row; `compact` shows the model, then the mode icon, a context meter, and tokens in/out on one row, then cache and TTL (the meter needs a sidebar of about 32 columns) |
