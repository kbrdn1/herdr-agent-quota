...
impl Sidebar {
  ...
  fn with_prompt_cache(style: PercentStyle, prompt_cache: serde_json::Value) -> Self {
    ...
    let snapshot =
      herdr_agent_quota::providers::claude::parse_statusline(&payload, CacheStore::now_unix())
        .unwrap();
    cache
...
fn gauges_window_rows_fit_herdr_content_width_in_both_percentage_modes() {
  ...
  for style in [PercentStyle::Remaining, PercentStyle::Used] {
    for width in 18..=40 {
      ...
      let tokens = MetadataTokens::from_snapshot_for_session(&snapshot, 0, None, style, shape);
