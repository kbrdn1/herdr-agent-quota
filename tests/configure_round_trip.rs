...
fn default_herdr_rows_become_plane_provider_usage_and_topic_lines() {
  ...
  assert!(applied.contains("fg = \"#86e89a\""));
  assert!(applied.contains("fg = \"#ffdf61\""));
  assert!(!applied.contains("fg = \"#c6d768\""));
  assert!(!applied.contains("fg = \"#e2bd58\""));
  assert!(applied.contains("fg = \"#ff7a7a\""));
  ...
  assert!(applied.contains("fg = \"#d4825d\""));
  assert!(applied.contains("fg = \"#7ab8ff\""));
  assert!(applied.contains("fg = \"#e0e0e0\""));
  assert!(applied.contains("fg = \"#c79bff\""));
  assert!(applied.contains("fg = \"#c79bff\""));
  assert!(applied.contains("fg = \"#8abfb8\""));
}
...
fn install_logged_herdr_and_codex(
  ...
  let pane_current_branch = pane_current
    .map(|json| format!("elif [ \"$1 $2\" = \"pane current\" ]; then\n  printf '%s\\n' '{json}'\n"))
    .unwrap_or_default();
  fs::write(
        &herdr,
        format!(
...
fn pi_codex_event_overlays_exact_session_context_and_cache_without_inventing_ttl() {
  ...
  let inventory = pi_inventory(&session, "{}");
  let (herdr, herdr_log, codex, _) = install_logged_herdr_and_codex(state.path(), &inventory, None);
  let output = run_pi_event(
    state.path(),
    &herdr,
    &codex,
