...
impl MetadataTokens {
  ...
  pub fn from_snapshot_for_pane(
    ...
    let context = session_id.and_then(|session_id| snapshot.context_for_session(Some(session_id)));
...
fn missing_five_hour_severity(provider: Provider, quota_5h: &str) -> Option<Severity> {
  (quota_5h == "5h N/A" && missing_five_hour_label(provider).is_some()).then_some(Severity::Unknown)
}
...
pub(crate) fn sidebar_context(
  ...
  // The meter is added at publish, once the rest of its row is known.
  if shape.layout == SidebarLayout::Compact {
    return format!("{}%", format_percent(context_percent(context, style)));
  }
...
mod tests {
  ...
  fn the_used_style_shortens_the_gauges_meter_without_changing_its_colour() {
    ...
    let used =
      MetadataTokens::from_snapshot_for_session(&snapshot, 0, None, PercentStyle::Used, gauges(26));
    assert_eq!(
      used.quota_5h,
  ...
  fn the_gauges_context_row_prints_through_the_chosen_percent_style() {
    ...
    let used =
      MetadataTokens::from_snapshot_for_session(&snapshot, 0, None, PercentStyle::Used, gauges(36));
    assert_eq!(
            used.quota_context,
  ...
  fn packed_and_stacked_print_context_used_under_either_percent_style() {
    ...
    for layout in [SidebarLayout::Packed, SidebarLayout::Stacked] {
      for style in [PercentStyle::Remaining, PercentStyle::Used] {
        let plain =
          MetadataTokens::from_snapshot_for_session(&snapshot, 0, None, style, layout.into());
  ...
  fn a_gauges_sidebar_too_narrow_for_a_meter_still_prints_the_chosen_percent() {
    ...
    let used =
      MetadataTokens::from_snapshot_for_session(&snapshot, 0, None, PercentStyle::Used, gauges(18));
  ...
  fn every_gauges_row_prints_the_same_quantity_when_the_style_flips() {
    ...
    .with_context(Some(crate::model::ContextUsage::new(43.0).unwrap()));
    let used =
      MetadataTokens::from_snapshot_for_session(&snapshot, 0, None, PercentStyle::Used, gauges(30));
  ...
  /// Compact drops the `cx` label and the mode's name. The meter is fitted
  /// at publish, so the token leaves here as the bare number.
  #[test]
  fn compact_renders_the_mode_icon_and_the_bare_context_number() {
    ...
    let compact = MetadataTokens::from_snapshot_for_session(
      &snapshot,
      0,
      None,
      PercentStyle::Used,
      SidebarShape::new(SidebarLayout::Compact, 32),
    );
  ...
  #[test]
  fn the_compact_meter_takes_what_the_row_leaves_and_refits_cleanly() {
    // 28 columns, 18 taken by the icon, `↑158k` and `↓1k` and their
    // separators: six cells, a space, `31%`.
    let fitted = fit_compact_context("31%", 18, 28);
    assert_eq!(
      fitted,
