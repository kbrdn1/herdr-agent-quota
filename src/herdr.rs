...
pub fn current_focused_pane() -> Result<Option<(String, Harness)>> {
  ...
  let value: Value = serde_json::from_slice(&output.stdout).context("parse focused Herdr pane")?;
...
fn fold_cache_row(tokens: &mut BTreeMap<String, String>, row: RowStyle) {
  ...
  let (Some(cache), Some(ttl)) = (tokens.get("quota_cache"), tokens.get("quota_cache_ttl")) else {
    return;
  };
...
const MODE_TOKEN_NAMES: [&str; 3] = [
  "quota_mode_normal",
  "quota_mode_warning",
  "quota_mode_danger",
];

/// One name at a time, cleared like the context row: switching from `auto` to
/// `bypass` moves the value to another token, and a pane that kept the old one
/// would claim to be in two modes at once.
fn insert_mode_token(
  tokens: &mut BTreeMap<String, String>,
  value: &str,
  severity: Option<crate::model::Severity>,
) {
  for name in MODE_TOKEN_NAMES {
    tokens.remove(name);
  }
  if value.trim().is_empty() {
    return;
  ...
  let name = match severity {
    Some(crate::model::Severity::Warning) => "quota_mode_warning",
    Some(crate::model::Severity::Danger) => "quota_mode_danger",
    _ => "quota_mode_normal",
  };
...
mod tests {
  ...
  fn cache_diagnostics_stay_inside_herdr_metadata_cap() {
    let snapshot = crate::model::ProviderSnapshot::new(
      Provider::Claude,
      vec![
        crate::model::UsageWindow::new(crate::model::WindowKind::FiveHour, 20.0, None).unwrap(),
        crate::model::UsageWindow::new(crate::model::WindowKind::Weekly, 30.0, None).unwrap(),
      ],
      0,
    )
    .with_context(Some(
