...
use crate::herdr::{
  current_focused_pane, find_agent_pane, list_agent_panes, list_agent_state, plugin_quota_present,
...
fn refresh_working_panes(
  ...
  let mut selected = panes
    .iter()
    .zip(routes)
    .filter(|(pane, resolution)| {
...
fn resolved_pane_tokens(
  ...
  let mut quota = match resolution {
    Resolution::Subscription(target) if target.credential_scope == CredentialScope::OMP_STORE => {
...
fn refresh_omp_target(
  ...
  let Some(account) = omp_provider::select_account(&usage, evidence.account_pin.as_deref()) else {
  ...
  match fetched {
    Ok(fetched) => {
      ...
      if preserve_context {
        ...
        cache.save_preserving_diagnostics_for_sessions(&mut snapshot, &session_ids, mtime)?;
      } else {
        cache.save(&snapshot)?;
      }
      Ok(ProviderOutcome {
...
fn load_statusline_snapshot(cache: &CacheStore, provider: Provider) -> Result<FetchedSnapshot> {
  ...
  if !snapshot.session_quota_only {
    ...
    snapshot = match provider {
      ...
      Provider::Agy => crate::providers::agy::parse_statusline(&value, snapshot.fetched_at_unix)?,
      _ => snapshot,
    };
  }
...
fn publish_resolved(
  ...
  for pane in panes.iter_mut() {
    ...
    if let Some(pane_tokens) = resolved_pane_tokens(cache, pane, resolved, now, row, force_target)?
    {
...
fn tokens_for_provider(
  ...
  snapshot.map(|snapshot| {
    MetadataTokens::from_snapshot_for_pane(snapshot, now_unix, session_id, row.percent, row.shape)
  })
}
...
mod tests {
  ...
  fn failed_new_login_attempts_are_debounced_without_reusing_old_quota() {
    ...
    for provider in [
      ...
      cache
        .save(&ProviderSnapshot::new(provider, vec![], 90).with_account_id(Some("old".into())))
        .unwrap();
      cache
        .mark_refresh_account(provider, 100, Some("old"))
        .unwrap();
      assert!(
        !should_skip_fetch_for_account(&cache, provider, false, 110, Some("new"), None).unwrap()
      );
      cache
        .mark_refresh_account(provider, 110, Some("new"))
        .unwrap();
      assert!(
        should_skip_fetch_for_account(&cache, provider, false, 120, Some("new"), None).unwrap()
      );
      assert!(!cache
        .load(provider)
        .unwrap()
      ...
      assert!(
        !should_skip_fetch_for_account(&cache, provider, false, 170, Some("new"), None).unwrap()
      );
      assert!(
        !should_skip_fetch_for_account(&cache, provider, true, 120, Some("new"), None).unwrap()
      );
    }
  }
  ...
  fn an_idle_pane_already_showing_the_cached_windows_stays_out_of_the_pass() {
    ...
    let row = publish_row(&cache);
    let values =
      MetadataTokens::from_snapshot_for_pane(&snapshot, 1_001, Some("s1"), row.percent, row.shape);
    assert_eq!(
  ...
  fn an_expired_cached_window_bypasses_the_fetch_debounce() {
      ...
      .chain(std::iter::once(Provider::OpenCodeGo))
    {
      ...
      cache
        ...
        .unwrap();
      assert!(
        !should_skip_fetch_for_account(&cache, provider, false, 1_001, Some("acc"), None).unwrap(),
      ...
      cache
        ...
        .unwrap();
      assert!(
        should_skip_fetch_for_account(&cache, provider, false, 1_030, Some("acc"), None).unwrap(),
  ...
  fn an_event_without_a_pane_reads_no_pane_at_all() {
    let value: Value = serde_json::from_str(r#"{"event":"x","data":{"agent":"claude"}}"#).unwrap();
