...
const QUOTA_SAFE_COLOR: &str = "#86e89a";
const QUOTA_WARNING_COLOR: &str = "#ffdf61";
const QUOTA_DANGER_COLOR: &str = "#ff7a7a";
...
const GAUGE_QUOTA_SAFE_COLOR: &str = "#8ec89a";
const GAUGE_QUOTA_WARNING_COLOR: &str = "#d6c377";
const GAUGE_QUOTA_DANGER_COLOR: &str = "#d68686";
...
// Branch is metadata, not a reading: grey, like the session id and version in
// the session's own statusLine. Traffic is a volume, not a verdict, so it gets
...
const PROVIDER_STYLES: [(Harness, &str, Option<&str>, Option<&str>); 9] = [
  ...
  (Harness::Grok, "grok", Some("#e0e0e0"), Some("#b0b0b0")),
  (Harness::Agy, "agy", Some("#c79bff"), Some("#d5b4ff")),
  ...
  (Harness::Pi, "pi", Some("#c79bff"), None),
  (Harness::Omp, "omp", Some("#c79bff"), None),
  (Harness::Devin, "devin", Some("#7ab8ff"), None),
  (Harness::Muse, "muse", Some("#7ab8ff"), None),
];
...
fn is_default_layout(rows: &Array, allow_legacy: bool) -> bool {
  ...
  match native.as_slice() {
    [] => true,
    [identity] => {
      is_default_state_equivalent(identity) || (allow_legacy && is_legacy_identity_row(identity))
    }
    [identity, agent] => {
...
/// The permission mode, on the band its blast radius earns: `plan` and
/// `default` ask before acting, `bypass` does not.
fn append_mode_row(rows: &mut Array, layout: SidebarLayout) {
  let mut row = Array::new();
  for (suffix, hex) in ["normal", "warning", "danger"]
    .into_iter()
    .zip(mode_palette(layout))
  {
    row.push(styled_token(
      &format!("$quota_mode_{suffix}"),
      Some(hex),
      Some(false),
      Some(false),
    ));
...
fn append_packed_quota_rows(rows: &mut Array) {
  ...
  // Same order as the other layouts: context, then the two counters that
  // explain it, then the quota windows.
...
fn append_stacked_quota_rows(rows: &mut Array, layout: SidebarLayout) {
  ...
  // Context first, then the traffic and cache readings under it: the row the
  // user watches leads, and the two counters that explain it follow.
  match layout {
...
"#;
  ...
  fn stacked_layout_puts_each_quota_field_on_its_own_row() {
    let original = "[ui.sidebar.agents]\nrows = [[\"state_icon\", \"agent\"]]\n";
    let updated =
      add_quota_row_for(original, &AgentSelection::SUPPORTED, SidebarLayout::Stacked).unwrap();
    assert_eq!(
      add_quota_row_for(&updated, &AgentSelection::SUPPORTED, SidebarLayout::Stacked).unwrap(),
      updated
    );
    let document = updated.parse::<DocumentMut>().unwrap();
    let rows = document["ui"]["sidebar"]["agents"]["rows"]
    ...
    let mode_index = rows
      .iter()
      .position(|row| row_contains_token(row, "$quota_mode_normal"))
      .unwrap();
    let topic_index = rows
      .iter()
      .position(|row| row_contains_token(row, "$quota_topic"))
      .unwrap();
    let branch_index = rows
      .iter()
      .position(|row| row_contains_token(row, "$quota_branch"))
      .unwrap();
    assert_eq!(identity_index + 1, provider_index);
    assert_eq!(provider_index + 1, model_index);
    // The mode sits under the model it applies to, and the branch under the
    // topic, immediately above the context row.
    assert_eq!(model_index + 1, mode_index);
    assert_eq!(mode_index + 1, topic_index);
    assert_eq!(topic_index + 1, branch_index);
    assert!(!rows
      .iter()
      .any(|row| row_contains_token(row, "$quota_provider_model")));
  ...
  fn gauges_gives_the_context_row_its_own_severity_colours() {
    let updated = add_quota_row_for("", &AgentSelection::SUPPORTED, SidebarLayout::Gauges).unwrap();
    let document = updated.parse::<DocumentMut>().unwrap();
    let rows = document["ui"]["sidebar"]["agents"]["rows"]
      .as_array()
      .unwrap();
  ...
  fn switching_between_any_two_layouts_is_idempotent() {
    ...
    let apply =
      |input: &str, layout| add_quota_row_for(input, &AgentSelection::SUPPORTED, layout).unwrap();
  ...
  fn only_gauges_publishes_the_muted_severity_palette() {
    ...
    let gauged = add_quota_row_for("", &AgentSelection::SUPPORTED, SidebarLayout::Gauges).unwrap();
    for hex in [
      GAUGE_QUOTA_SAFE_COLOR,
      GAUGE_QUOTA_WARNING_COLOR,
      GAUGE_QUOTA_DANGER_COLOR,
  ...
  fn gives_no_cached_a_red_token_without_spending_an_extra_metadata_slot() {
    ...
    assert!(updated.contains("fg = \"#ffdf61\""));
  }
...
"#;
    ...
    assert!(
      !updated.contains("opencode = [[\"state_icon\", \"agent\"]] # herdr-agent-quota-provider")
    );
    let removed = remove_quota_row(&updated).unwrap();
...
"#;
  ...
  fn custom_rows_with_brand_off_still_render_quota_in_provider_rows() {
    ...
    assert!(
      !claude_fgs.iter().any(|fg| fg == "#d4825d"),
...
mod field_tests {
  ...
  fn omp_uses_the_previous_opencode_violet_and_opencode_is_neutral() {
    ...
    assert!(omp.lines().next().unwrap_or_default().contains("#c79bff"));
