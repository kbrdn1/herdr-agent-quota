...
use crate::cli::{
  AgentOrder, AgentSelection, BrandColors, ConfigureOptions, FieldSet, LowQuotaAlert, PercentStyle,
...
pub fn run(
  ...
  if uninstall {
    ...
    let interval = options
      ...
      .or_else(|| prefs::read(prefs::WATCH_INTERVAL_SECONDS).and_then(|value| value.parse().ok()));
...
pub(crate) fn resolved_sidebar_layout(
  ...
  SidebarLayout::from_arg_or_env(explicit)
    .or_else(|| prefs::read(prefs::SIDEBAR_LAYOUT).and_then(|value| SidebarLayout::parse(&value)))
...
pub(crate) fn resolved_low_quota_alert(
  ...
  LowQuotaAlert::from_arg_or_env(explicit)
    .or_else(|| prefs::read(prefs::LOW_QUOTA_ALERT).and_then(|value| LowQuotaAlert::parse(&value)))
