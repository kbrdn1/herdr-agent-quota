...
fn map_request_error(error: &ureq::Error) -> ProviderError {
  match error {
    ureq::Error::Status(401, _) | ureq::Error::Status(403, _) => ProviderError::MissingCredentials,
...
mod tests {
  ...
  fn write_sessions_db(dir: &Path, rows: &[(&str, &str)]) -> PathBuf {
    ...
    conn
      .execute_batch("CREATE TABLE sessions (id TEXT, model TEXT)")
      .unwrap();
  ...
  fn session_models_from_db_returns_err_for_missing_table() {
    ...
    let conn = Connection::open(&db_path).unwrap();
    conn
      .execute_batch("CREATE TABLE wrong_table (id TEXT)")
      .unwrap();
  ...
  fn session_models_from_db_ignores_extra_columns() {
    ...
    let conn = Connection::open(&db_path).unwrap();
    conn
      .execute_batch("CREATE TABLE sessions (id TEXT, model TEXT, created_at TEXT, workspace TEXT)")
      .unwrap();
    conn
      .execute(
      ...
      .unwrap();
    let models = session_models_from_db(&db_path, &["dapper-indigo".to_string()]).expect("db read");
    assert_eq!(
      models,
