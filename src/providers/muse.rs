...
impl SessionObservation {
  ...
  fn context(&self, session_id: &str, limits: &BTreeMap<String, u64>) -> Option<ContextUsage> {
    ...
    let cache =
      CacheUsage::from_token_counts(fresh, usage.cache_read, usage.cache_write).map(|cache| {
...
fn observe_session_tail(tail: &str) -> SessionObservation {
  ...
  for line in tail.lines().rev() {
    ...
    if record.get("payload_type").and_then(Value::as_str) == Some("runtime.user_intent.accepted") {
      if wants_prompt {
...
mod tests {
  ...
  fn an_account_without_a_subscription_keeps_local_fields_only_for_a_session() {
    ...
    let never = |_: &MuseCredentials, _| -> std::result::Result<ProviderSnapshot, ProviderError> {
