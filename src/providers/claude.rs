...
fn parse_window(
  ...
  let used = value
    ...
    .ok_or_else(|| ProviderError::UnsupportedResponse(format!("missing {} usage", kind.label())))?;
...
pub fn run_statusline(input: &[u8]) -> std::result::Result<ProviderSnapshot, ProviderError> {
  let value: Value = serde_json::from_slice(input)
    .map_err(|_| ProviderError::UnsupportedResponse("statusLine input is not JSON".to_string()))?;
