...
pub fn resolve_with_identity(pane: &AgentPane) -> ResolvedPane {
  let resolution = match pane.harness {
    ...
    Harness::Omp => {
      return resolve_omp_with_identity(pane.session.as_ref().and_then(|session| session.path()))
    }
  };
  ResolvedPane {
    resolution,
...
mod tests {
  ...
  fn missing_session_is_indeterminate_even_with_exactly_one_credential() {
    let auth = parse_auth_json(br#"{"opencode-go":{"type":"api","key":"placeholder"}}"#).unwrap();
