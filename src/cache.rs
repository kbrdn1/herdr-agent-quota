  ...
  let resolved_scope = session_id.and_then(|session_id| {
    quota_scope.map(str::to_string).or_else(|| {
      snapshot
        ...
        .cloned()
        .or_else(|| {
          previous.and_then(|previous| previous.session_quota_scopes.get(session_id).cloned())
        })
    })
  });
    ...
    match (session_id, resolved_scope.as_deref()) {
      (Some(session_id), scope) => {
        let previous_windows =
          previous_windows_for_merge(previous, session_id, scope).map(|windows| windows.to_vec());
...
fn merge_profile_quota_windows(
  ...
  .into_iter()
  .filter_map(|kind| merge_profile_quota_window(window_in(stored, kind), window_in(current, kind)))
  .collect()
}
...
mod tests {
  ...
  use crate::model::{
    window_in, BillingTarget, CacheUsage, ContextUsage, Provider, ResetAt, UsageWindow, WindowKind,
  };
  use serde_json::json;
  ...
  fn original_four_snapshots_use_canonical_0_2_cache_filenames() {
    ...
    for (provider, filename) in [
      ...
      let loaded: ProviderSnapshot = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
  ...
  fn statusline_observations_keep_models_for_multiple_sessions() {
    ...
    cache
      .save_statusline_observation(
        Provider::Claude,
        ProviderSnapshot::new(Provider::Claude, vec![], 1).with_model(Some("Sonnet".to_string())),
        &json!({"session_id": "session-1"}),
      )
      .unwrap();
    cache
      .save_statusline_observation(
        Provider::Claude,
        ProviderSnapshot::new(Provider::Claude, vec![], 2).with_model(Some("Opus".to_string())),
  ...
  fn merge_profile_quota_windows_accepts_a_newer_reset_and_rejects_an_older_one() {
    let newer = merge_profile_quota_windows(&[five_hour(92.0, 16_000)], &[five_hour(5.0, 34_000)]);
    assert_eq!(newer[0].used_percent, 5.0);
    assert_eq!(newer[0].resets_at, Some(ResetAt::from_unix_seconds(34_000)));

    let older = merge_profile_quota_windows(&[five_hour(20.0, 34_000)], &[five_hour(95.0, 16_000)]);
  ...
  fn direct_provider_refresh_does_not_leak_global_context_to_a_new_session() {
    ...
    cache
      .save_preserving_diagnostics_for_sessions(&mut latest, &["new-session".to_string()], None)
      .unwrap();
    let saved = cache.load(Provider::Grok).unwrap().unwrap();
  ...
  fn direct_provider_refresh_does_not_restore_five_hour_window_after_account_switch() {
    ...
    let cache = CacheStore::new(directory.path());
    cache
      .save(&codex_windows(Some(80.0), 31.0, 1_000).with_account_id(Some("account-a".to_string())))
      .unwrap();
  ...
  fn a_fresh_api_read_removes_legacy_unattributed_windows() {
    ...
    let cache = CacheStore::new(directory.path());
    cache
      .save(&codex_windows(Some(22.0), 65.0, 1_000).with_account_id(Some("account-a".to_string())))
      .unwrap();
  ...
  fn watch_interval_rejects_values_that_are_too_short_or_long() {
    assert!(CacheStore::validate_watch_interval_seconds(MIN_WATCH_INTERVAL_SECONDS - 1).is_err());
    assert!(CacheStore::validate_watch_interval_seconds(MAX_WATCH_INTERVAL_SECONDS + 1).is_err());
