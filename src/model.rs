...
impl<'de> Deserialize<'de> for ResetAt {
    ...
    D: serde::Deserializer<'de>,
  {
    ...
    match Repr::deserialize(deserializer)? {
      ...
      Repr::Text(value) => Self::parse(&value)
        .ok_or_else(|| serde::de::Error::custom("reset time is not Unix seconds or RFC 3339")),
    }
  }
}
...
impl ProviderSnapshot {
  ...
  pub fn usable_for_account(
    ...
    match (self.account_id.as_deref(), current_account_id) {
      ...
      (Some(_), None) => false,
      (None, Some(_)) => false,
      (None, None) => credentials_mtime_unix.is_none_or(|mtime| mtime <= self.fetched_at_unix),
    }
  }
  ...
  pub fn severity_for_windows(
    ...
    let relevant = match provider {
      ...
      | Provider::Omp
      | Provider::Devin
      | Provider::Muse => window_in(&live, WindowKind::FiveHour).or_else(|| long_window(&live)),
    };
    relevant
...
mod tests {
  ...
  fn claude_known_session_does_not_borrow_the_provider_model() {
    let mut snapshot =
      ProviderSnapshot::new(Provider::Claude, vec![], 0).with_model(Some("latest".to_string()));
    snapshot
      .session_models
